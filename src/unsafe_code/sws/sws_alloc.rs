...
pub fn create_sws_context<'a>(height: i32, width: i32, in_pix_fmt: AVPixelFormat, out_pix_fmt: AVPixelFormat) -> Result<&'a mut SwsContext, UnsafeError> {
    unsafe {
...
unsafe fn scale_using_sws(old_frame: &mut AVFrame, sws_context: &mut SwsContext, align: i32, pts: i64) -> Result<*mut AVFrame, UnsafeError> {
    ...
    let _ = sws_scale(sws_context, raw_frame_data_ptr, raw_frame_linesize_ptr, 0, old_frame.height, scaled_frame_data_ptr, scaled_frame_linesize_ptr);
